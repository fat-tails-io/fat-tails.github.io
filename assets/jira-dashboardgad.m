...
// -----------------------------------------------------------------------------
(dashboardId as text, optional moduleKey as text, optional uri as text, optional gadgetId as text) =>
let
    // CONFIGURATION setup. 
    ...
    QueryParams = [
            moduleKey = if moduleKey = null then null else moduleKey,
            uri = if uri = null then null else uri,
            gadgetId = if gadgetId = null then null else gadgetId,
        ],
    ...
    // Build URL
    BaseUrl = JiraBaseUrl & "/rest/api/3/dashboard/" & dashboardId & "/gadget",
