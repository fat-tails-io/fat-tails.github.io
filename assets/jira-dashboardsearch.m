 ...
 *  Dashboards shared with a private project that the user can browse. Not returned for anonymous users.
 *  Dashboards shared with a public project.
 *  Dashboards shared with the public. with automatic pagination handling
// API Endpoint: GET /rest/api/3/dashboard/search
// Create this with query name: JiraGetDashboardSearch
// -----------------------------------------------------------------------------
(optional dashboardName as text, optional accountId as text, optional owner as text, optional groupname as text, optional groupId as text, optional projectId as text, optional orderBy as text, optional startAt as text, optional maxResults as text, optional status as text, optional expand as text) =>
let
    // CONFIGURATION setup. 
    ...
    QueryParams = [
            dashboardName = if dashboardName = null then null else dashboardName,
        ...
        CleanQueryParams = Record.SelectFields(
            ...
            BaseUrl = JiraBaseUrl & "/rest/api/3/dashboard/search",
