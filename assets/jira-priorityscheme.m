...
// -----------------------------------------------------------------------------
(optional startAt as text, optional maxResults as text, optional priorityId as text, optional schemeId as text, optional schemeName as text, optional onlyDefault as text, optional orderBy as text, optional expand as text) =>
let
    // CONFIGURATION setup. 
    ...
    QueryParams = [
            ...
            priorityId = if priorityId = null then null else priorityId,
            schemeId = if schemeId = null then null else schemeId,
            schemeName = if schemeName = null then null else schemeName,
            onlyDefault = if onlyDefault = null then null else onlyDefault,
    ...
    // Build URL
    BaseUrl = JiraBaseUrl & "/rest/api/3/priorityscheme",
