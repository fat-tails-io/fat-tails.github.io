 ...
 *  *Browse projects* [project permission](https://confluence.atlassian.com/x/yodKLg) for the project containing the issue.
 *  If [issue-level security](https://confluence.atlassian.com/x/J4lKLg) is configured, issue-level security permission to view the issue.
// API Endpoint: GET /rest/api/3/issue/{issueIdOrKey}/properties/{propertyKey}
// Create this with query name: JiraGetIssueProperties
// -----------------------------------------------------------------------------
(issueIdOrKey as text, propertyKey as text) =>
let
    // CONFIGURATION setup. 
    ...
    // Build URL
    BaseUrl = JiraBaseUrl & "/rest/api/3/issue/" & issueIdOrKey & "/properties/" & propertyKey & "",
    FullUrl = BaseUrl,
