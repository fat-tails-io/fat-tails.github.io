 ...
 *  If [issue-level security](https://confluence.atlassian.com/x/J4lKLg) is configured, issue-level security permission to view the issue.
 *  If the comment has visibility restrictions, the user belongs to the group or has the role visibility is restricted to.
// API Endpoint: GET /rest/api/3/issue/{issueIdOrKey}/comment/{id}
// Create this with query name: JiraGetIssueComment
// -----------------------------------------------------------------------------
(issueIdOrKey as text, id as text, optional expand as text) =>
let
    // CONFIGURATION setup. 
    ...
    // Build URL
    BaseUrl = JiraBaseUrl & "/rest/api/3/issue/" & issueIdOrKey & "/comment/" & id & "",
