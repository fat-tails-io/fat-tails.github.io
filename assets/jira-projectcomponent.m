...
**[Permissions](#permissions) required:** *Browse Projects* [project permission](https://confluence.atlassian.com/x/yodKLg) for the project.
// API Endpoint: GET /rest/api/3/project/{projectIdOrKey}/component
// Create this with query name: JiraGetProjectComponent
// -----------------------------------------------------------------------------
(projectIdOrKey as text, optional startAt as text, optional maxResults as text, optional orderBy as text, optional componentSource as text, optional query as text) =>
let
    // CONFIGURATION setup. 
    ...
    QueryParams = [
            ...
            componentSource = if componentSource = null then null else componentSource,
    ...
    // Build URL
    BaseUrl = JiraBaseUrl & "/rest/api/3/project/" & projectIdOrKey & "/component",
