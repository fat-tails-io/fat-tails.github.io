...
// Description: |-
  ...
  **[Permissions](#permissions) required:** *Browse users and groups* [global permission](https://confluence.atlassian.com/x/x4dKLg). Anonymous calls or calls by users without the required permission return empty search results. with automatic pagination handling
// API Endpoint: GET /rest/api/3/user/search
// Create this with query name: JiraGetUserSearch
// -----------------------------------------------------------------------------
(optional query as text, optional username as text, optional accountId as text, optional startAt as text, optional maxResults as text, optional property as text) =>
let
    // CONFIGURATION setup. 
    ...
    QueryParams = [
            ...
            property = if property = null then null else property,
        ],
        ...
        CleanQueryParams = Record.SelectFields(
            ...
            BaseUrl = JiraBaseUrl & "/rest/api/3/user/search",
