 ...
 *  Browse [project permission](https://support.atlassian.com/jira-cloud-administration/docs/manage-project-permissions/) in all projects that contain the selected issues.
 *  If [issue-level security](https://confluence.atlassian.com/x/J4lKLg) is configured, issue-level security permission to view the issue.
 *  Depending on the field, any field-specific permissions required to edit it. with automatic pagination handling
// API Endpoint: GET /rest/api/3/bulk/issues/fields
// Create this with query name: JiraGetBulkIssuesFields
// -----------------------------------------------------------------------------
(issueIdsOrKeys as text, optional searchText as text, optional endingBefore as text, optional startingAfter as text) =>
let
    // CONFIGURATION setup. 
    ...
    QueryParams = [
            ...
            searchText = if searchText = null then null else searchText,
            endingBefore = if endingBefore = null then null else endingBefore,
            startingAfter = if startingAfter = null then null else startingAfter,
        ],
        ...
        CleanQueryParams = Record.SelectFields(
            ...
            BaseUrl = JiraBaseUrl & "/rest/api/3/bulk/issues/fields",
