...
// -----------------------------------------------------------------------------
(id as text, optional redirect as text, optional fallbackToDefault as text, optional width as text, optional height as text) =>
let
    // CONFIGURATION setup. 
    ...
    QueryParams = [
            redirect = if redirect = null then null else redirect,
            fallbackToDefault = if fallbackToDefault = null then null else fallbackToDefault,
            width = if width = null then null else width,
            height = if height = null then null else height,
        ],
    ...
    // Build URL
    BaseUrl = JiraBaseUrl & "/rest/api/3/attachment/thumbnail/" & id & "",
