...
// -----------------------------------------------------------------------------
(optional startAt as text, optional maxResults as text, optional orderBy as text, optional id as text, optional keys as text, optional query as text, optional typeKey as text, optional categoryId as text, optional action as text, optional expand as text, optional status as text, optional properties as text, optional propertyQuery as text) =>
let
    // CONFIGURATION setup. 
    ...
    QueryParams = [
            ...
            keys = if keys = null then null else keys,
            query = if query = null then null else query,
            typeKey = if typeKey = null then null else typeKey,
            categoryId = if categoryId = null then null else categoryId,
            action = if action = null then null else action,
            expand = if expand = null then null else expand,
            status = if status = null then null else status,
            properties = if properties = null then null else properties,
            propertyQuery = if propertyQuery = null then null else propertyQuery,
        ],
        ...
        CleanQueryParams = Record.SelectFields(
            ...
            BaseUrl = JiraBaseUrl & "/rest/api/3/project/search",
