...
**[Permissions](#permissions) required:** *Browse users and groups* [global permission](https://confluence.atlassian.com/x/x4dKLg). Anonymous calls and calls by users without the required permission return empty search results. with automatic pagination handling
// API Endpoint: GET /rest/api/3/user/viewissue/search
// Create this with query name: JiraGetUserViewissueSearch
// -----------------------------------------------------------------------------
(optional query as text, optional username as text, optional accountId as text, optional issueKey as text, optional projectKey as text, optional startAt as text, optional maxResults as text) =>
let
    // CONFIGURATION setup. 
    ...
    QueryParams = [
        ...
        CleanQueryParams = Record.SelectFields(
            ...
            BaseUrl = JiraBaseUrl & "/rest/api/3/user/viewissue/search",
