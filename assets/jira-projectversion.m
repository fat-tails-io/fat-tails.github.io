...
// -----------------------------------------------------------------------------
// JiraGetProjectVersion
// Description: Returns a [paginated](#pagination) list of all versions in a project. See the [Get project versions](#api-rest-api-3-project-projectIdOrKey-versions-get) resource if you want to get a full list of versions without pagination.

This operation can be accessed anonymously.

**[Permissions](#permissions) required:** *Browse Projects* [project permission](https://confluence.atlassian.com/x/yodKLg) for the project.
// API Endpoint: GET /rest/api/3/project/{projectIdOrKey}/version
// Create this with query name: JiraGetProjectVersion
// -----------------------------------------------------------------------------
(projectIdOrKey as text, optional startAt as text, optional maxResults as text, optional orderBy as text, optional query as text, optional status as text, optional expand as text) =>
let
    // CONFIGURATION setup. 
    ...
    // Build URL
    BaseUrl = JiraBaseUrl & "/rest/api/3/project/" & projectIdOrKey & "/version",
