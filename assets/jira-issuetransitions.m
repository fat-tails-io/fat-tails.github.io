...
// -----------------------------------------------------------------------------
(issueIdOrKey as text, optional expand as text, optional transitionId as text, optional skipRemoteOnlyCondition as text, optional includeUnavailableTransitions as text, optional sortByOpsBarAndStatus as text) =>
let
    // CONFIGURATION setup. 
    ...
    QueryParams = [
            ...
            transitionId = if transitionId = null then null else transitionId,
            skipRemoteOnlyCondition = if skipRemoteOnlyCondition = null then null else skipRemoteOnlyCondition,
            includeUnavailableTransitions = if includeUnavailableTransitions = null then null else includeUnavailableTransitions,
            sortByOpsBarAndStatus = if sortByOpsBarAndStatus = null then null else sortByOpsBarAndStatus,
        ],
    ...
    // Build URL
    BaseUrl = JiraBaseUrl & "/rest/api/3/issue/" & issueIdOrKey & "/transitions",
