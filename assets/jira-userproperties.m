 ...
 *  *Administer Jira* [global permission](https://confluence.atlassian.com/x/x4dKLg), to get a property from any user.
 *  Access to Jira, to get a property from the calling user's record.
// API Endpoint: GET /rest/api/3/user/properties/{propertyKey}
// Create this with query name: JiraGetUserProperties
// -----------------------------------------------------------------------------
(propertyKey as text, optional accountId as text, optional userKey as text, optional username as text) =>
let
    // CONFIGURATION setup. 
    ...
    QueryParams = [
            ...
            userKey = if userKey = null then null else userKey,
    ...
    // Build URL
    BaseUrl = JiraBaseUrl & "/rest/api/3/user/properties/" & propertyKey & "",
