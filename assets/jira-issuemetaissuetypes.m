---
title: Get Create Field Metadata For A Project And Issue Type Id
...
// -----------------------------------------------------------------------------
// JiraGetIssueCreatemetaIssuetypes
// Description: Returns a page of field metadata for a specified project and issuetype id. Use the information to populate the requests in [ Create issue](#api-rest-api-3-issue-post) and [Create issues](#api-rest-api-3-issue-bulk-post).

This operation can be accessed anonymously.

**[Permissions](#permissions) required:** *Create issues* [project permission](https://confluence.atlassian.com/x/yodKLg) in the requested projects.
// API Endpoint: GET /rest/api/3/issue/createmeta/{projectIdOrKey}/issuetypes/{issueTypeId}
// Create this with query name: JiraGetIssueCreatemetaIssuetypes
// -----------------------------------------------------------------------------
(projectIdOrKey as text, issueTypeId as text, optional startAt as text, optional maxResults as text) =>
let
    // CONFIGURATION setup. 
    ...
    // Build URL
    BaseUrl = JiraBaseUrl & "/rest/api/3/issue/createmeta/" & projectIdOrKey & "/issuetypes/" & issueTypeId & "",
