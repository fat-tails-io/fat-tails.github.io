...
// API Endpoint: GET /rest/api/3/user/search/query/key
// Create this with query name: JiraGetUserSearchQueryKey
// -----------------------------------------------------------------------------
(query as text, optional startAt as text, optional maxResult as text) =>
let
    // CONFIGURATION setup. 
    ...
    QueryParams = [
            ...
            maxResult = if maxResult = null then null else maxResult,
        ],
        ...
        CleanQueryParams = Record.SelectFields(
            ...
            BaseUrl = JiraBaseUrl & "/rest/api/3/user/search/query/key",
